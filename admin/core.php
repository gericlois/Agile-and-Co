...
foreach ($coreRows as $cp) {
    $validPages[] = $cp['slug'];
    $pageLabels[$cp['slug']] = $cp['label'];
    if ($cp['is_custom']) {
        $previewLinks[$cp['slug']] = $customPreviewLinks[$cp['slug']] ?? '../core-page.php?p=' . $cp['slug'];
        $customSlugs[] = $cp['slug'];
    } else {
        $previewLinks[$cp['slug']] = '../core-page.php?p=' . $cp['slug'];
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'add_core_page') {
    ...
    if (empty($newLabel) || empty($newSlug)) {
        ...
        $error = 'A core page with this slug already exists.';
    } else {
        try {
            $maxOrder = $pdo->query("SELECT MAX(sort_order) FROM core_pages")->fetchColumn();
            $stmt = $pdo->prepare("INSERT INTO core_pages (slug, label, is_custom, sort_order) VALUES (?, ?, 0, ?)");
            ...
            $insertStmt->execute([$newSlug . '_hero', 'pre_headline', $newLabel]);
            $insertStmt->execute([$newSlug . '_hero', 'headline', $newLabel]);
            $insertStmt->execute([$newSlug . '_hero', 'headline_accent', 'Powered by AI.']);
            $insertStmt->execute([$newSlug . '_hero', 'subtitle', 'Learn more about our ' . strtolower($newLabel) . ' technology.']);
            ...
            $error = 'Error adding core page. Please try again.';
        }
    }
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'delete_core_page') {
    ...
    if (!empty($deleteSlug) && !in_array($deleteSlug, $customSlugs)) {
        try {
            $stmt = $pdo->prepare("DELETE FROM core_pages WHERE slug = ? AND is_custom = 0");
            $stmt->execute([$deleteSlug]);

            // Delete all site_content rows for this page
            $stmt = $pdo->prepare("DELETE FROM site_content WHERE section LIKE ?");
            $stmt->execute([$deleteSlug . '_%']);

            logActivity($pdo, 'delete', 'core_page', 'Deleted core page: ' . $deleteSlug);
            $success = 'Core page deleted successfully.';

            header('Location: core.php?deleted=1');
            exit;
        } catch (PDOException $e) {
            $error = 'Error deleting core page. Please try again.';
        }
    } else {
        $error = 'Cannot delete built-in core pages.';
    }
}
...
// Current tab
$currentPage = $_GET['page'] ?? ($validPages[0] ?? 'main');
if (!in_array($currentPage, $validPages)) {
    $currentPage = $validPages[0] ?? 'main';
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && ($_POST['action'] ?? '') === 'save_content') {
    try {
        ...
        logActivity($pdo, 'update', 'content', 'Updated core content: ' . $pageLabels[$currentPage]);
...
// Build standard section definitions for new (non-custom) core pages
function buildCoreSections($slug) {
    return [
        $slug . '_hero' => [
            'label' => 'Hero Section',
            'fields' => [
...
// Hardcoded section definitions for the original Core page
$customPageSections = [
    'main' => [
        'core_hero' => [
            'label' => 'Hero Section',
        ...
        'core_problem' => [
            ...
            'fields' => [
                'pre_headline' => ['label' => 'Pre-headline', 'type' => 'text'],
                'headline' => ['label' => 'Headline (HTML allowed)', 'type' => 'textarea'],
                'month1_label' => ['label' => 'Timeline 1 Label', 'type' => 'text'],
                'month1_text' => ['label' => 'Timeline 1 Text', 'type' => 'textarea'],
                'month2_label' => ['label' => 'Timeline 2 Label', 'type' => 'text'],
                'month2_text' => ['label' => 'Timeline 2 Text', 'type' => 'textarea'],
                'month3_label' => ['label' => 'Timeline 3 Label', 'type' => 'text'],
                'month3_text' => ['label' => 'Timeline 3 Text', 'type' => 'textarea'],
                'month4_label' => ['label' => 'Timeline 4 Label', 'type' => 'text'],
                'month4_text' => ['label' => 'Timeline 4 Text', 'type' => 'textarea'],
                'conclusion_text' => ['label' => 'Conclusion Text', 'type' => 'textarea'],
                'conclusion_bold' => ['label' => 'Conclusion Bold Text', 'type' => 'text'],
            ],
        ],
        'core_features' => [
            'label' => 'Features Section (How Core Works)',
            'fields' => [
                'pre_headline' => ['label' => 'Pre-headline', 'type' => 'text'],
                'headline' => ['label' => 'Headline (HTML allowed)', 'type' => 'textarea'],
                'intro' => ['label' => 'Intro Paragraph', 'type' => 'textarea'],
                'f1_tech' => ['label' => 'Feature 1 Tech Label', 'type' => 'text'],
                'f1_title' => ['label' => 'Feature 1 Title', 'type' => 'text'],
                'f1_text1' => ['label' => 'Feature 1 Paragraph 1', 'type' => 'textarea'],
                'f1_text2' => ['label' => 'Feature 1 Paragraph 2', 'type' => 'textarea'],
                'f1_example' => ['label' => 'Feature 1 Example', 'type' => 'textarea'],
                'f2_tech' => ['label' => 'Feature 2 Tech Label', 'type' => 'text'],
                'f2_title' => ['label' => 'Feature 2 Title', 'type' => 'text'],
                'f2_text1' => ['label' => 'Feature 2 Paragraph 1', 'type' => 'textarea'],
                'f2_text2' => ['label' => 'Feature 2 Paragraph 2', 'type' => 'textarea'],
                'f2_example' => ['label' => 'Feature 2 Example', 'type' => 'textarea'],
                'f3_tech' => ['label' => 'Feature 3 Tech Label', 'type' => 'text'],
                'f3_title' => ['label' => 'Feature 3 Title', 'type' => 'text'],
                'f3_text1' => ['label' => 'Feature 3 Paragraph 1', 'type' => 'textarea'],
                'f3_text2' => ['label' => 'Feature 3 Paragraph 2', 'type' => 'textarea'],
                'f3_example' => ['label' => 'Feature 3 Example', 'type' => 'textarea'],
            ],
        ],
        'core_results' => [
...
if ($isCustom && isset($customPageSections[$currentPage])) {
    ...
    $sections = buildCoreSections($currentPage);
    $currentDefaults = [];
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>Edit Core | Agile & Co Admin</title>
...
<body>
    <div class="admin-layout">
        <aside class="admin-sidebar">
            ...
            <nav class="admin-nav">
                ...
                <a href="core.php" class="admin-nav-item active">
        ...
        <main class="admin-main">
            <div class="admin-header">
                <div>
                    <h1>Edit Core</h1>
                    <p>Manage the content displayed on Core pages</p>
                </div>
                <button onclick="document.getElementById('addCoreForm').style.display = document.getElementById('addCoreForm').style.display === 'none' ? 'block' : 'none'" class="btn btn-primary" style="padding: 10px 20px; font-size: 14px;">+ Add Core Page</button>
            </div>
                ...
                <form method="POST" action="core.php">
                    ...
                    <div style="display: grid; grid-template-columns: 1fr 1fr auto; gap: 16px; align-items: end;">
                        <div class="form-group" style="margin-bottom: 0;">
                            ...
                            <input type="text" name="page_name" placeholder="e.g. AI Analytics" required oninput="this.form.page_slug.value = this.value.toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/^-|-$/g, '')">
                        </div>
                        <div class="form-group" style="margin-bottom: 0;">
                            <label>URL Slug</label>
                            <input type="text" name="page_slug" placeholder="e.g. ai-analytics" required pattern="[a-z0-9\-]+">
                        </div>
                        <button type="submit" class="btn btn-primary" style="padding: 12px 24px; white-space: nowrap;">Create Page</button>
                    </div>
                    <p style="color: var(--gray-400); font-size: 13px; margin-top: 12px;">The page will be accessible at: core-page.php?p=<span id="slugPreview" style="color: var(--accent);">slug</span></p>
                </form>
            </div>
            ...
            <div class="admin-tabs">
                <?php foreach ($pageLabels as $key => $label): ?>
                    <a href="core.php?page=<?= $key ?>" class="admin-tab <?= $currentPage === $key ? 'active' : '' ?>"><?= htmlspecialchars($label) ?></a>
                <?php endforeach; ?>
            </div>
                        ...
                        <div class="admin-accordion-body">
                            <?php foreach ($sectionData['fields'] as $fieldKey => $fieldInfo): ?>
                                <div class="form-group">
                                    ...
                                    <?php
                                    $defaultValue = $currentDefaults[$sectionKey][$fieldKey] ?? '';
    ...
    <script>
    // Update slug preview in add form
